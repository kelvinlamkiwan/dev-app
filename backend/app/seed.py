...
def seed_data():
    ...
    try:
        ...
        if db.query(models.Style).count() == 0:
            ...
            db.add(models.BomItem(bom_id=bom.id, component_id=comp["Vamp"].id, material_id=mat["Cow Leather Black"].id, quantity=2.0, unit="sqft", unit_cost=2.5, color="Black", sort_order=1))
            db.add(models.BomItem(bom_id=bom.id, component_id=comp["Sole"].id, material_id=mat["Rubber Outsole White"].id, quantity=1.0, unit="pair", unit_cost=3.0, color="White", sort_order=2))
            db.add(models.BomItem(bom_id=bom.id, component_id=comp["Midsole"].id, material_id=mat["EVA Midsole White"].id, quantity=1.0, unit="pair", unit_cost=1.5, color="White", sort_order=3))
            db.commit()
    finally:
        db.close()
